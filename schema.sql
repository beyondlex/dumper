...
ALTER TABLE `schema_attendance` ADD COLUMN `rule_id` INT NULL COMMENT '考勤规则id' AFTER `user_id`,
 ADD COLUMN `apply_id` VARCHAR (200) NULL COMMENT '对考勤有影响的申请id集合' AFTER `cross_day`,
 ...
 ADD COLUMN `attendance_apply` VARCHAR (255) NULL COMMENT '关联申请' AFTER `ot_least_time`,
 ADD COLUMN `record_way` VARCHAR (255) NULL COMMENT '允许打卡方式' AFTER `attendance_apply`,
 ADD COLUMN `rule_type` TINYINT (4) NULL COMMENT '班制类型。1固定2多班3自由' AFTER `record_way`,
...
ALTER TABLE `schema_schedule` ADD COLUMN `rest` TINYINT (1) NULL DEFAULT 1 COMMENT '是否有休息时间，1是0否' AFTER `cross_day`,
 ...
 MODIFY COLUMN `schedule_name` VARCHAR (50) CHARACTER
SET utf8 COLLATE utf8_general_ci NULL COMMENT '班次名称' AFTER `department_id`,
...
ALTER TABLE `schema_apply_config` 
-- DROP COLUMN `apply_enname`,
 -- DROP COLUMN `url`,
 MODIFY COLUMN `department_id` VARCHAR (100) NULL DEFAULT NULL COMMENT '可使用此申请的部门id' AFTER `type`,
 MODIFY COLUMN `approver` VARCHAR (255) CHARACTER
SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '申请审批人id' AFTER `apply_name`,
...
ALTER TABLE `schema_apply` MODIFY COLUMN `content` text CHARACTER
SET utf8 COLLATE utf8_general_ci NULL COMMENT '内容' AFTER `title`,
...
ALTER TABLE `schema_notice` MODIFY COLUMN `content` VARCHAR (1000) CHARACTER
SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT '公告内容' AFTER `title`,
